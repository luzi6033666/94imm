...
BASE_URL = "https://www.xiaomiaoshe.com"
REQUEST_HEADERS = {
    "User-Agent": (
...
PAGE_LIMIT = max(int(os.environ.get("CRAWLER_XIAOMIAOSHE_PAGES", "1")), 1)
POST_LIMIT = max(int(os.environ.get("CRAWLER_XIAOMIAOSHE_POST_LIMIT", "1")), 1)
SCAN_LIMIT = max(int(os.environ.get("CRAWLER_XIAOMIAOSHE_SCAN_LIMIT", str(POST_LIMIT * 3))), POST_LIMIT)
SOURCE_PATHS = [
    item.strip()
    for item in os.environ.get("CRAWLER_XIAOMIAOSHE_PATHS", "/cosplay/").split(",")
    if item.strip()
]
IMAGE_HOST_ALLOWLIST = ("image.xiaomiaoshe.com",)


def normalize_title(title):
    return re.sub(r"\s*[–-]\s*小喵社\s*$", "", (title or "").strip())
    ...
    for page_number in range(1, PAGE_LIMIT + 1):
        ...
        for anchor in soup.select("a[href]"):
            ...
            if not path or "/" in path:
                continue
            if path in {"cosplay", "mote", "archives", "jieyajiaocheng", "lianxikefu", "user-login", "zhuce"}:
                continue
            if href in seen:
                continue
            seen.add(href)
...
def parse_detail(session, detail_url, source_path):
    ...
    date_node = soup.select_one("meta[property='article:published_time'], meta[property='og:updated_time']")
    ...
    image_urls = []
    seen_images = set()
    for image in soup.select(".entry-content img[data-src], .entry-content img[src], .article-content img[data-src], .article-content img[src], article img[data-src], article img[src]"):
