...
BASE_URL = "https://www.miaoyinshe.com"
REQUEST_HEADERS = {
    "User-Agent": (
...
PAGE_LIMIT = max(int(os.environ.get("CRAWLER_MIAOYINSHE_PAGES", "1")), 1)
POST_LIMIT = max(int(os.environ.get("CRAWLER_MIAOYINSHE_POST_LIMIT", "1")), 1)
SCAN_LIMIT = max(int(os.environ.get("CRAWLER_MIAOYINSHE_SCAN_LIMIT", str(POST_LIMIT * 3))), POST_LIMIT)
SOURCE_PATHS = [
    item.strip()
    for item in os.environ.get("CRAWLER_MIAOYINSHE_PATHS", "/cosplay").split(",")
    if item.strip()
]
IMAGE_HOST_ALLOWLIST = ("image.miaoyinshe.com",)
IMAGE_PATH_BLOCKLIST = (
    "/vip.jpg",
    "cropped-images-removebg-preview",
)


def normalize_title(title):
    return re.sub(r"\s*[–-]\s*喵映社\s*$", "", (title or "").strip())
...
def collect_post_urls(session, source_path):
    ...
    for page_number in range(1, PAGE_LIMIT + 1):
        ...
        for anchor in soup.select(".post-list-item .post-info h2 a[href], .post-list-item .thumb-link[href]"):
...
def parse_detail(session, detail_url, source_path):
    ...
    article_text = (soup.select_one(".entry-content") or soup.select_one("article") or soup).get_text(" ", strip=True)

    image_urls = []
    seen_images = set()
    for image in soup.select(".entry-content img[data-src], .entry-content img[src]"):
        ...
        if urlparse(image_url).netloc not in IMAGE_HOST_ALLOWLIST:
            continue
        if any(marker in image_url for marker in IMAGE_PATH_BLOCKLIST):
            continue
    ...
    if len(image_urls) < 3:
        if "合集目录" in article_text or "作品推荐" in article_text:
            print("目录页跳过：" + detail_url)
        else:
