#!/opt/mm187/.venv/bin/python
import os
import re
import sys
from urllib.parse import urlparse

from bs4 import BeautifulSoup

sys.path.append(os.path.dirname(os.path.dirname(__file__)))
os.environ.setdefault("DJANGO_SETTINGS_MODULE", "silumz.settings")

import django

django.setup()

from common import build_session, fetch
from gallery_source import download_and_store_gallery, parse_date, run_source_paths


BASE_URL = "https://coserlab.io"
REQUEST_HEADERS = {
    "User-Agent": (
        "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 "
        "(KHTML, like Gecko) Chrome/124.0.0.0 Safari/537.36"
    ),
    "Referer": BASE_URL,
}
PAGE_LIMIT = max(int(os.environ.get("CRAWLER_COSERLAB_PAGES", "1")), 1)
POST_LIMIT = max(int(os.environ.get("CRAWLER_COSERLAB_POST_LIMIT", "1")), 1)
SCAN_LIMIT = max(int(os.environ.get("CRAWLER_COSERLAB_SCAN_LIMIT", str(POST_LIMIT * 3))), POST_LIMIT)
SOURCE_PATHS = [
    item.strip()
    for item in os.environ.get("CRAWLER_COSERLAB_PATHS", "/archives/category/cosplay").split(",")
    if item.strip()
]
IMAGE_HOST_ALLOWLIST = ("autoselect.nian.nz",)


def normalize_title(title):
    title = re.sub(r"\s*-\s*Coser Lab\s*$", "", (title or "").strip(), flags=re.I)
    return re.sub(r"\s+", " ", title)


def infer_type_id(source_path):
    return 6 if "cosplay" in source_path else 1
def build_list_url(source_path, page_number):
    base = BASE_URL + source_path
    return base if page_number == 1 else "{}/page/{}".format(base, page_number)


def extract_post_id(post_url):
    match = re.search(r"/archives/(\d+)$", post_url)
    return int(match.group(1)) if match else 0


def collect_post_urls(session, source_path):
    seen = set()
    post_urls = []

    for page_number in range(1, PAGE_LIMIT + 1):
        list_url = build_list_url(source_path, page_number)
        response = fetch(session, list_url, headers=REQUEST_HEADERS)
        if response is None:
            continue

        soup = BeautifulSoup(response.text, "html.parser")
        for anchor in soup.select("a[href*='/archives/']"):
            href = anchor.get("href", "").strip()
            if not href:
                continue
            url = href if href.startswith("http") else BASE_URL + href
            if not re.search(r"/archives/\d+$", url):
                continue
            if url in seen:
                continue
            seen.add(url)
            post_urls.append(url)

    post_urls.sort(key=extract_post_id, reverse=True)
    return post_urls[:SCAN_LIMIT]


def parse_detail(session, detail_url, source_path):
    response = fetch(session, detail_url, headers=REQUEST_HEADERS, referer=BASE_URL)
    if response is None:
        return None

    soup = BeautifulSoup(response.text, "html.parser")
    title_node = soup.select_one("meta[property='og:title']")
    title = normalize_title(title_node.get("content", "") if title_node else (soup.title.get_text(" ", strip=True) if soup.title else ""))
    if not title:
        print("详情页结构异常：" + detail_url)
        return None

    date_node = soup.select_one("meta[property='article:published_time']")
    send_date = parse_date(date_node.get("content", "")) if date_node else parse_date("")

    tags = [source_path.rsplit("/", 1)[-1]]
    image_urls = []
    seen_images = set()
    for image in soup.select("img.masonry-image[data-src], img.masonry-image[src]"):
        image_url = image.get("data-src") or image.get("src") or ""
        image_url = image_url.strip()
        if not image_url.startswith("http"):
            continue
        host = urlparse(image_url).netloc
        if host not in IMAGE_HOST_ALLOWLIST:
            continue
        if image_url in seen_images:
            continue
        seen_images.add(image_url)
        image_urls.append(image_url)

    if len(image_urls) < 5:
        print("详情页图片不足：" + detail_url)
        return None

    return {
        "title": title,
        "send_date": send_date,
        "tags": tags,
        "type_id": infer_type_id(source_path),
        "image_urls": image_urls,
    }


def download_and_store(session, detail_url, source_path):
    parsed = parse_detail(session, detail_url, source_path)
    if not parsed:
        return False

    return download_and_store_gallery(session, detail_url, parsed, REQUEST_HEADERS)


def main():
    session = build_session(headers=REQUEST_HEADERS, retries=2)
    run_source_paths(
        SOURCE_PATHS,
        lambda source_path: collect_post_urls(session, source_path),
        lambda post_url, source_path: download_and_store(session, post_url, source_path),
        POST_LIMIT,
    )


if __name__ == "__main__":
    main()
